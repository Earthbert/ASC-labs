...
// database has empty(0) and non-empty(!=0) positions, around DB_ELEM/DB_SIZE
// occupied change the kernel so each thread writes dataToAdd in an empty spot
// in database (0 1 0 1 0 ) => (t1:2, t2:3) => (2 1 1 3 0)
__global__ void kernel_write(unsigned long long int *data,
	unsigned long long int *database, int dbElemNum) {
	...
	for (int i = 0; i < dbElemNum; i++) {
		long long unsigned res = atomicExch(&database[i], dataToAdd);
		if (res == 0) {
			break;
		}
	}
...
void validateDB(unsigned long long int *database, int dbSize, int expNZElem) {
	...
	if (!isValid) {
		...
		cout << endl
			<< "INVALID, expected database elements " << expNZElem << " but got "
			<< numNZElem << endl;
	} else {
